...
- You github credentials are set up in RStudio Cloud, see [here](https://rstudio.cloud/learn/guide#github-details)
...
```{r}
beauty %>%summarise(no_of_female_professors=sum(female))
```
...
4: This allows us to see the number of female professors at each age. However, i have a feeling this is not what i need to do.
...
Answer: Slightly Positive, this is due to the slight ascension in the line of best fit as we cross the student evaluation of teaching score. However, this is only a slight increase and largely minimal, indicating that the results are nearly random.
